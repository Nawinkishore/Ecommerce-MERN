...
const ProductDetail = () => {
    const { product, loading, error } = useSelector((state) => state.productState); // Destructure the product, loading, and error from state.product
    const { id } = useParams(); // Destructure the id from useParams
    const dispatch = useDispatch();
    const [qty, setQty] = useState(0);

    useEffect(() => {
        dispatch(getProduct(id)); // Pass the id directly
    }, [dispatch, id]);
    ...
    return (
        <div>
            <div className={'w-screen h-full flex justify-evenly p-5'}>
                <img src="/images/ps5.jpg" alt="Product" />
                <div>
                    {loading ? <Loader /> : (
                        <div className={'-ml-20'}>
